...
(function($) {
	...
	function timechange() {
		...
		addMinute = ltx.Tmin;
		addHour = ltx.Thour
		newTimeHms=ltx.StringTime
		changeType = true
		console.log(changeType)
	}

	changeNum=function(data) {
		// h:m:s  时间固定状态码  length  大于5属于带有小时状态  
		var h, m, s, hNum, mNum, sNum, timeNum;
		if(data.length > 5) {
			var p = data.indexOf(":")
			h = data.slice(0, p)
			var dm = data.indexOf(":", p + 1);
			m = data.slice(p + 1, dm);
			var sm = data.indexOf(":", dm + 1)
			s = data.slice(dm + 1, sm);
			console.log(h, m, s)
			// 时间计算
			hNum = h * 60 * 60 * 5;
			mNum = m * 60 * 5;
			sNum = s * 5;
			timeNum = hNum + mNum + sNum
		}
		return timeNum
	}
