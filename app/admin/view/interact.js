layui.define("form", function(exports) {
	...
	interact.prototype.select = function(e, obj, value) {
		...
		if(value !== "") {
			var i = otl.index();
			if(!ot || !ot.length) {
				ot = otl.after('<div class="layui-input-inline"><select name="' + e.name + '[]"><option value="">' + (hints[index] && hints[index][i] ? hints[index][i] : "璇烽€夋嫨") + "</option></select></div>").next()
			}
	...
	interact.prototype.selected = function(e, arr, hint) {
		...
		o.each(e.selected, function(idx, value) {
			if(idx < e.selected.length - 1) {
				...
				arr.push('<div class="layui-input-inline"><select name="' + e.name + '[]"><option value="">' + (hint[idx] ? hint[idx] : "璇烽€夋嫨") + "</option>" + options + "</select></div>")
			}
		});
		return arr
	};
